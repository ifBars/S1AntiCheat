...
internal sealed class VerificationService
{
    ...
    private void OnClientMessage(string payload)
    {
        if (InstanceFinder.IsHost || !WireCodec.TryDecodeChallenge(payload, out string nonce))
        {
            ...
            {
                MelonLogger.Msg($"{Constants.LogPrefix} Host verification {(allowed ? "passed" : "failed")}: {message}");
            }
        ...
        {
            MelonLogger.Warning($"{Constants.LogPrefix} Could not submit the client verification report.");
        }
    }
    ...
    private void OnServerMessage(NetworkConnection connection, string payload)
    {
        ...
        if (!Version.TryParse(runtimeVersion, out Version? clientVersion) ||
            clientVersion.CompareTo(new Version(Constants.ModVersion)) < 0)
        {
            Reject(peer, $"S1 Anti-Cheat {Constants.ModVersion} or newer is required.");
            return;
        }
        ...
        MelonLogger.Msg($"{Constants.LogPrefix} Verified SteamID {peer.SteamId} (connection {peer.ConnectionId}, {mods.Count} mod(s)).");
    }
    ...
    private void Reject(PeerState peer, string reason)
    {
        ...
        MelonLogger.Warning($"{Constants.LogPrefix} Rejected SteamID {peer.SteamId} (connection {peer.ConnectionId}): {reason}");
    }
}
